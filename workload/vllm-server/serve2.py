...
def load_trace_intervals_first_n(
    csv_path: str,
    n: int,
    ...
    """
    CSV에서 앞 n행의 TIMESTAMP만 읽고, inter-arrival interval(초) 리스트를 만든다.
    반환 길이 = n (첫 요청은 0.0, 이후는 delta_t)
    """
    if n <= 0:
    ...
    with open(csv_path, "r", encoding="utf-8") as f:
        ...
        if timestamp_col not in (reader.fieldnames or []):
            raise ValueError(f"CSV missing column '{timestamp_col}'")
        for row in reader:
            if len(ts_list) >= n:
                break
    ...
    if len(ts_list) < n:
        raise ValueError(
            f"CSV rows available ({len(ts_list)}) < requested n ({n})."
        )
    ...
    for i in range(1, n):
        ...
        if delta < 0:
            delta = 0.0
        intervals.append(delta / speedup)
...
async def get_request(
    ...
    trace_intervals: Optional[list[float]] = None,
) -> AsyncGenerator[tuple[str, int, int], None]:
    """
    input_requests(프롬프트/토큰길이 생성)은 기존대로 유지.
    trace_intervals가 주어지면, 요청 간 sleep을 trace 기반으로 한다.
    """
    assert burstiness > 0, (
        f"A positive burstiness factor is expected, but given {burstiness}."
    )

    if trace_intervals is not None:
        if len(trace_intervals) < len(input_requests):
            raise ValueError(
                f"trace_intervals length ({len(trace_intervals)}) must be >= "
                f"num requests ({len(input_requests)})."
            )

        for i, req in enumerate(input_requests):
            yield req
            interval = trace_intervals[i]
            if interval > 0:
...
async def benchmark(
    ...
    trace_intervals: Optional[list[float]] = None,  # 추가
):
    ...
    async for request in get_request(input_requests, request_rate, burstiness, trace_intervals=trace_intervals):
...
def add_cli_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--trace-path",
        type=str,
        default="/data/25-2/trace/AzureLLMInferenceTrace_conv_1week.csv",
        help="CSV trace path. If set, request inter-arrival times follow the first N rows' TIMESTAMP deltas.",
    )
    parser.add_argument(
        "--trace-speedup",
        type=float,
        default=1.0,
        help="Speed up the trace timeline by this factor (e.g., 10 => 10x faster).",
    )
...
def main(args: argparse.Namespace):
    ...
    trace_intervals = None
    if args.trace_path is not None:
        trace_intervals = load_trace_intervals_first_n(
            csv_path=args.trace_path,
            n=args.num_prompts,          # <- N = num_prompts (앞 N행 사용)
